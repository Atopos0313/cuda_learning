...
constexpr int kElementCount =
    kMatrixExtent * kMatrixExtent;
...
int main()
{
    try {
        std::vector<float> input(
            static_cast<std::size_t>(kElementCount)
        );
        ...
        {
            const int pattern =
                static_cast<int>(index % 2048) - 1024;
            input[index] =
        ...
        const int blocks =
            (kElementCount + kThreadsPerBlock - 1)
            / kThreadsPerBlock;

        copy_contiguous_kernel<<<
            blocks,
            kThreadsPerBlock
        >>>(
            d_input.data(),
