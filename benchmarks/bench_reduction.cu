...
{
    return operation == ReductionOperation::Sum
        ? "sum"
        : "max";
}
...
{
    return variant == ReductionVariant::Shared
        ? "shared"
        : "warp";
}
...
)
{
    ...
    const double reference =
        cpu_reference(operation, h_input);

    DeviceBuffer<float> d_input(
        static_cast<std::size_t>(n)
    );
    ...
    const double million_elements_per_second =
        ...
        (static_cast<double>(pipeline_ms) * 1.0e3);

    const double effective_bandwidth_gbs =
        effective_global_bytes(
            n,
            threads_per_block
        ) /
...
)
{
    ...
    std::cout
        ...
        << std::setprecision(9)
        << result.reference << ","
        << static_cast<double>(result.result)
        << ","
        << std::scientific
...
int main()
{
    const std::vector<ReductionOperation>
        operations = {
    ...
    std::cout
        << "operation,variant,N,block,rounds,"
